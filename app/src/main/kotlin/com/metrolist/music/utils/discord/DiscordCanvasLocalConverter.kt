...
object DiscordCanvasLocalConverter {
    ...
    suspend fun prepareAndUpload(
        ...
        cachedCanvasUrl(uploadUrl)?.let { cachedUrl ->
            if (isUploadedCanvasReady(cachedUrl, client)) {
    ...
    private fun cachedCanvasUrl(uploadUrl: String): String? {
        val marker = "/apple_canvas/upload/"
        val index = uploadUrl.indexOf(marker)
        if (index < 0) return null
        return uploadUrl.replace(marker, "/apple_canvas/")
    }

    private fun isUploadedCanvasReady(
        url: String,
        client: OkHttpClient,
    ): Boolean =
        runCatching {
            ...
            client.newCall(request).execute().use { response ->
                response.isSuccessful &&
                    response.header("Content-Type").orEmpty().contains("image/gif", ignoreCase = true)
            }
        }.getOrDefault(false)
